...
const addSubtitle = async(req,res) => {
   ...
   // const corporateProgress= await CorporateTraineeProgress.find({"CourseId":mongoose.Types.ObjectId(courseId)},{SubtitleId:1}).populate('SubtitleId')
    }else{
...
const getNumberofSubtitlies = async (req,res) => { 
    try{
        console.log(req.query.CourseId);
        const data=await course.find({ "_id":req.query.CourseId},{SubtitleId:1}).count()
        console.log(data)
        res.status(200).json(data);
    }
    catch(error){
...
//         let updated;
//         let old
//         const NumberofSubtitlies=await subtitle.find({ "CourseId":req.query.CourseId},{_id:1}).count();
//         const n =1/NumberofSubtitlies;
//         old= await course.findOne({"_id":req.query.CourseId},{Progress:1,_id:0});
//         const checkStatus=await subtitle.findOne({"CourseId":req.query.CourseId,"_id":req.query.SubtitleId},{ProgressStatus:1,_id:0})

//         if(checkStatus.ProgressStatus==false){

//         data=await subtitle.findOneAndUpdate({"CourseId":req.query.CourseId,"_id":req.query.SubtitleId},{ProgressStatus:flag})
//         const p= await course.findOne({"_id":req.query.CourseId},{Progress:1,_id:0})
//         const pp=p.Progress;
//         const ppp=pp+n*100;
//         updateP=await course.findByIdAndUpdate({"_id":req.query.CourseId},{Progress:ppp})
//         updated=await course.findOne({"_id":req.query.CourseId},{Progress:1,_id:0})
//         res.status(200).json(updated);
//         console.log(updated);
//         }
//         else{
...
module.exports = {View_All_Courses, Filter_By_Subject, Filter_By_Rate, Filter_By_Price,data,createCourse,addANewInstructor,Search_By_Instructor_Name,Search_By_Title,Filter_By_Subject_And_Price,Filter_By_Subject_And_Rating,Filter_By_Subject_And_Rating_And_Price,viewMyInstructorCoursesById,getCurrentCourseDetails,getCurrentCourseInformation,addCourseDiscount,fetchCourseDiscountsByCourseId,addSubtitle,fetchSubtitlesByCourseId,fetchInstructorById,fetchCoursePreviewLink,getCurrentCourseInstructor,
    ...
    ratingACourse,fetchTheSubtitleBySubtitleId,getNumberofSubtitlies,getStatusOfSubtitlie,UpdateProgressOfSubtitlie,postMethod};